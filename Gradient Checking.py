...
def gradient_check(x, theta, epsilon = 1e-7):
    ...
    # Compute gradapprox using formula: dJ/dtheta=lim(epsilon -> 0) ( J(theta + epsilon) - J(theta-epsilon) ) / (2*epsion). 
    # Here epsilon is small enough, so  it can be used directly.
    ...
    # Checks if gradapprox is close enough to the output of backward_propagation().
...
def backward_propagation_n(X, Y, cache):
    ...
    db1 = 4./m * np.sum(dZ1, axis=1, keepdims = True)  #4./m..
